<template>
    <div class="content-main-holder">
        <div class="page-title-holder row">
            <div class="col-6 col"><h4>All products</h4></div>
            <div class="col-6 col"><router-link  class="btn mebtn-primary" :to="{name: 'Addproduct'}"><span>Add product</span></router-link></div>
        </div>
        <div class="main-table-hold mt-4">
            <div v-if="getProducts == ''" class="empthy-table-state">
                <div class="container">
                    <div>
                        <p class="p1">You don’t have any product uploaded yet!</p>
                        <p class="p2 pb-3">Begin by adding your first product.</p>
                        <p class="p3"><router-link class="btn mebtn-primary" :to="{name: 'Addproduct'}">Add product</router-link></p>
                    </div>
                </div>
            </div>
            <div v-else>
                ...
                <div class="table-responsive">
                        <table id="products" class="table align-items-center table-flush no-border">
                            ...
                            <tr>
                                ...
                                <th>
                                    <span v-show="!$store.getters.getAllchecks">Product</span>
                                    <div v-show="$store.getters.getAllchecks" class="table-actions">
                                        <ul>
                                            ...
                                            <li><a @click.prevent="$store.dispatch('bulkDelproducts')" href="#" class="del btn"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 12.2 16"><path class="icon delete-icon" d="M.87,14.22A1.77,1.77,0,0,0,2.61,16h7a1.76,1.76,0,0,0,1.74-1.78V3.56H.87ZM12.2.89h-3L8.28,0H3.92L3.05.89H0V2.67H12.2Z" transform="translate(0 0)"/></svg><span>Delete selection</span></a></li>
                                        </ul>
                                    </div>
                                </th>
                                <th><span v-show="!$store.getters.getAllchecks">Status</span></th>
                                <th><span v-show="!$store.getters.getAllchecks">Inventory</span></th>
                                <th><span v-show="!$store.getters.getAllchecks">Type</span></th>
                                <th><span v-show="!$store.getters.getAllchecks">Date created</span></th>
                            </tr>
                            </thead>
                            <tbody id="product_tbody">
                                <table-row  v-for="item in getProducts" :key="item.id" v-bind:item="item" />
                            </tbody>          
                        </table>
                </div>
...
import TableRow from '../includes/ProductTableRow.vue'
export default {
    name: 'Allproducts',
    components: {TableRow},
    computed: mapGetters(['getProducts', 'getCheckedItems', 'getCheckedItems', 'getToken']),
    data() {
        return {
            checked: false,
        me: false,
