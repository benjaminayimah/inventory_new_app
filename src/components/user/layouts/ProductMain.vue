<template>
    <div class="content-main-holder">
        <div class="page-title-holder row">
            <div class="col-6 col flex"><router-link :to="{name: 'Allproducts'}" class="btn mebtn-alt flex"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 22 18"><path class="icon-arrow-back" d="M22,7.5H5.74l5.37-5.39L9,0,0,9l9,9,2.11-2.12L5.74,10.5H22Z" transform="translate(0 0)"/></svg>Back</router-link><h4>Edit product</h4></div>
        </div>
        <alerts />
            ...
            <div class="mt-4 row">
                            ...
                            <div class="form-group">
                                ...
                                <div class="image-hold">
                                    ...
                                    <div v-if="imageUploaded" class="image-in flex">
                                        <div class="imghold-div" v-for="img in form.tempImage" :key="img.id" v-bind:style="{backgroundImage: 'url('+getHostname+'/storage/'+getUser.id+'/temp2/'+img.name+')'}">
                                            <button class="btn" @click.prevent="deltmp(img.name)">
                ...
                <div class="col-md-5">
                    <div class="inner-form-hold">
                        <div class="product-details-hold">
                            <div class="form-group">
                                <div class="update-labels"><div class="pull-left">Status:</div><div class="status-display pull-left" v-bind:class="[(thisStatus === 'active' ? 'status-active' : ''), (thisStatus === 'draft' ? 'status-draft' : ''), (thisStatus === 'disabled' ? 'status-disabled' : '')]" v-if="thisStatus"><span class="st-active">{{ thisStatus | capitalize}}</span></div></div>
                        ...
                        <div class="product-details-hold">
                            <div class="form-group">
                                ...
                                <div class="update-labels"><div class="pull-left">Category:</div><div class="category-display pull-left" v-if="thisCategory"><span>{{ thisCategory }}</span></div></div>
                                <select v-model="form.category" class="custom-select mr-sm-2 my-select" id="category">
                                    <option selected value="">Update category</option>
                ...
                <div class="mt-4 mb-4 container">
                    <a class="btn mebtn-seconday mr-3" href="#">Discard</a><a class="btn mebtn-primary" href="#" @click.prevent="submitUpdate()">Save</a> 
                </div>
            </div>
        </form>
...
export default {
  components: { Alerts },
    name: 'ProductMain',
    computed: mapGetters(['getToken', 'getHostname', 'getUser', 'getTagdrop']),
    data() {
        return {
            form: {
                name: '',
    ...
    methods: {
        ...
        getItem() {
                axios.post(this.getHostname+'/api/product-detail?token='+this.getToken,{
                    id: this.$route.params.id
                })
                .then((res) => {
                    this.form.name = res.data.item.name
                    this.form.description = res.data.item.description
                    this.form.type = res.data.item.product_type
                    this.form.price = res.data.item.price
                    this.form.compared = res.data.item.compared_as
                    this.form.cpu = res.data.item.cpu
                    this.form.sku = res.data.item.sku
                    this.form.profit = res.data.item.profit
                    this.form.profitMargin = res.data.item.profit_margin
                    this.form.barcode = res.data.item.barcode
                    if (res.data.item.continue_selling === 1) {
                        this.form.continue = true
                    }else{
                        this.form.continue = false
                    }
                    if (res.data.item.track_qty === 1) {
                        this.form.track = true
                    }else{
                        this.form.track = false
                    }
                    if (res.data.item.new_arrival === 1) {
                        this.form.tags.new = true
                    }else{
                        this.form.tags.new = false
                    }
                    if (res.data.item.best_seller === 1) {
                        this.form.tags.bs = true
                    }else{
                        this.form.tags.bs = false
                    }
                    this.categories = res.data.category
                    this.thisCategory = res.data.item.category
                    this.thisStatus = res.data.item.status
                    if(res.data.images.length > 0) {
                        this.form.tempImage = res.data.images
        ...
        uploadTemp() {
            ...
            axios.post( this.getHostname+'/api/temp-editupload?token='+this.getToken,
                    formData,
                    {
                        headers: {
        ...
        submitUpdate() {
            axios.put(this.getHostname+'/api/product-detail/'+this.$route.params.id+'?token='+this.getToken,
                    this.form,
                    {
                        headers: {
                ...
                this.form.category = ''
                this.form.status = ''
                if(res.data.thisCategory) {
                    this.thisCategory = res.data.thisCategory
                }if (res.data.thisStatus) {
                    this.thisStatus = res.data.thisStatus
                }
                const payload = {
                    id: 'success',
        ...
        deltmp(id) {
            //api call
            axios.delete(this.getHostname+'/api/temp-editupload/'+id+'?token='+this.getToken)
            .then(() => {
                this.form.tempImage = this.form.tempImage.filter(img => img.name !== id)
                if(this.form.tempImage.length < 1) {
                    this.imageUploaded = false
                }
                this.form.deleted.push(id)
            }).catch((err) => {
                console.log(err)
            })
        },
...
.update-labels div:first-child {
    color: rgba(21, 31, 57, 0.7);
    margin-right: 10px;
}
.update-labels div:nth-child(2) {
    border-radius: 16px;
    padding: 3px 14px;
    margin-top: -4px;
}
